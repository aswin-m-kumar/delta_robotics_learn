  ...
  return (
    <div className="flex flex-col h-full gap-6">
      <div className="flex justify-between items-end shrink-0">
        <div>
          <h2 className="text-2xl font-bold text-text-main">Workshops</h2>
          <p className="text-sm text-text-muted mt-1">14 Workshops total • Managing across 3 campuses</p>
        </div>
      </div>

      <div className="flex items-center gap-4 bg-bg-alt p-1.5 rounded-lg w-max shrink-0">
        <span className="text-xs font-semibold text-text-muted uppercase tracking-wider ml-2 mr-1">Show:</span>
        {tabs.map((tab) => (
          <button
            key={tab.id}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 overflow-y-auto pb-4">
        {filteredWorkshops.map((ws) => (
          <div key={ws.id} className="bg-white border border-border-main rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow group flex flex-col">
            <div className="h-40 bg-bg-alt relative shrink-0">
              <img src={ws.image} alt={ws.title} className="w-full h-full object-cover" />
            </div>
            
            <div className="p-5 flex flex-col flex-1">
              <h3 className="font-semibold text-text-main text-lg mb-4 line-clamp-1 group-hover:text-brand transition-colors">
                {ws.title}
              </h3>
              ...
              <div className="flex items-center justify-between mt-auto pt-4 border-t border-border-main">
                ...
                <div className="flex -space-x-2">
                  {ws.avatars.map((avatar, i) => (
                    <img key={i} src={avatar} alt="" className="w-6 h-6 rounded-full border-2 border-white object-cover bg-bg-alt" />
                  ))}
                  <div className="w-6 h-6 rounded-full border-2 border-white bg-brand-tint text-brand flex items-center justify-center text-[10px] font-bold">
      ...
      >
        <div className="space-y-6">
          <section className="space-y-2">
            <label className="text-xs font-semibold text-text-muted uppercase tracking-wider">Workshop Title</label>
            <input type="text" placeholder="e.g. Intro to ROS Navigation" className="w-full h-10 border border-border-main rounded-lg px-3 bg-white focus:outline-none focus:ring-2 focus:ring-brand text-sm" />
          </section>
          ...
          <div className="grid grid-cols-2 gap-4">
            <section className="space-y-2">
              ...
              <div className="relative">
                <input type="date" className="w-full h-10 border border-border-main rounded-lg px-3 bg-white focus:outline-none focus:ring-2 focus:ring-brand text-sm text-text-muted" />
              </div>
            </section>
            <section className="space-y-2">
              <label className="text-xs font-semibold text-text-muted uppercase tracking-wider">Venue</label>
              <select className="w-full h-10 border border-border-main rounded-lg px-3 bg-white focus:outline-none focus:ring-2 focus:ring-brand text-sm text-text-muted">
          ...
          <h3 className="font-semibold text-text-main text-sm uppercase tracking-wider">Media Assets</h3>

          <section className="space-y-2">
            <label className="text-[10px] font-bold text-text-muted uppercase tracking-wider">Poster Image</label>
            <div className="border-2 border-dashed border-border-main rounded-xl p-8 flex flex-col items-center justify-center text-center bg-bg-alt hover:bg-white transition-colors cursor-pointer">
              ...
              <p className="font-medium text-text-main text-sm"><span className="text-brand">Click to upload</span> or drag and drop</p>
              <p className="text-xs text-text-muted mt-1">Recommended size: 1200×800px</p>
            </div>
          </section>

          <section className="space-y-2">
            <label className="text-[10px] font-bold text-text-muted uppercase tracking-wider">Gallery Images (Optional)</label>
            <div className="w-24 h-24 border border-border-main rounded-xl flex items-center justify-center bg-bg-alt hover:bg-white transition-colors cursor-pointer text-text-muted hover:text-text-main">
              <Plus size={24} />
            </div>
          </section>
        </div>
